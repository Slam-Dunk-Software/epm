//! `epm runtime` — install and manage runtime tools (observatory + tree_walker).
//!
//! These are treated specially: they bypass the registry entirely and are
//! sourced directly from their canonical GitHub repos.
//!
//! Note: epc is no longer a separate binary — service management is now
//! built into epm itself as `epm services`.
...
pub enum RuntimeCommands {
    /// Scaffold observatory and install tree_walker
    Install {
        /// Only install a specific component: observatory or tree_walker
        #[arg(value_name = "COMPONENT")]
        component: Option<String>,
    },
    /// Upgrade a runtime component to the latest release
    Upgrade {
        /// Component to upgrade: tree_walker
        #[arg(value_name = "COMPONENT")]
        component: Option<String>,
    },
...
async fn run_install(only: Option<&str>) -> Result<()> {
    match only {
        Some("epc") => {
            println!("\x1b[33m!\x1b[0m \x1b[1mepc\x1b[0m is now built into \x1b[1mepm\x1b[0m — no separate install needed.");
            println!("  Use \x1b[36mepm services\x1b[0m to manage your EPS services.");
            Ok(())
        }
        Some("observatory") => install_observatory(),
        Some("tree_walker") => install_tree_walker().await,
        Some(other) => anyhow::bail!(
            "unknown component '{other}' — valid options: observatory, tree_walker"
        ),
        None => {
            install_observatory()?;
            println!();
...
fn install_observatory() -> Result<()> {
    ...
    if dest.exists() {
        println!(
            "\x1b[32m✓\x1b[0m \x1b[1mobservatory\x1b[0m \x1b[2malready exists at {}\x1b[0m",
            dest.display()
        );
        return Ok(());
    }
    ...
    std::fs::remove_dir_all(dest.join(".git")).context("failed to remove upstream .git")?;

    Command::new("git").args(["init", &dest_str]).stdout(Stdio::null()).stderr(Stdio::null()).status()?;
    Command::new("git").args(["-C", &dest_str, "add", "."]).stdout(Stdio::null()).stderr(Stdio::null()).status()?;
    Command::new("git")
        .args(["-C", &dest_str, "commit", "-m", "Initial commit (epm runtime install)"])
        .stdout(Stdio::null()).stderr(Stdio::null()).status()?;

    println!(
        "\x1b[32m✓\x1b[0m \x1b[1mobservatory\x1b[0m \x1b[2mready at {}\x1b[0m",
        dest.display()
    );
    println!();
    println!("  \x1b[2mDeploy it:\x1b[0m");
    println!("    \x1b[36mcd ~/observatory && epm services serve .\x1b[0m");
...
async fn install_tree_walker() -> Result<()> {
    ...
    if dest.exists() {
        ...
        println!(
            "\x1b[32m✓\x1b[0m \x1b[1mtree_walker\x1b[0m \x1b[2malready installed{}\x1b[0m",
    ...
    println!(
        "\x1b[32m✓\x1b[0m \x1b[1mtree_walker v{version}\x1b[0m \x1b[2minstalled to {}\x1b[0m",
        dest.display()
    );
...
async fn run_upgrade(only: Option<&str>) -> Result<()> {
    match only {
        Some("epc") => {
            println!("\x1b[33m!\x1b[0m \x1b[1mepc\x1b[0m is now built into \x1b[1mepm\x1b[0m.");
            println!("  Upgrade epm itself with: \x1b[36mepm self-update\x1b[0m");
            Ok(())
        }
        Some("observatory") => {
            println!("\x1b[2mobservatory is your harness — you own the source.\x1b[0m");
            println!("\x1b[2mTo pick up changes after editing, run:\x1b[0m \x1b[36mepm services restart observatory\x1b[0m");
            Ok(())
        }
        Some("tree_walker") | None => upgrade_tree_walker().await,
        Some(other) => anyhow::bail!(
            "unknown component '{other}' — valid options: tree_walker"
        ),
    }
}
...
async fn upgrade_tree_walker() -> Result<()> {
    ...
    println!(
        "\x1b[2mUpdating tree_walker\x1b[0m \x1b[1mv{current_str}\x1b[0m \x1b[2m→\x1b[0m \x1b[1mv{latest}\x1b[0m\x1b[2m...\x1b[0m"
    );

    let asset = tree_walker_asset_name();
    let download_url = release_asset_url(&client, TREE_WALKER_REPO, &latest, asset).await?;

    let dest = if let Ok(p) = which::which("tree_walker") { p } else { std::path::PathBuf::from("/usr/local/bin/tree_walker") };
...
async fn run_status() -> Result<()> {
    ...
    print_status("observatory", obs_current.as_deref(), obs_latest.as_deref(), "cd ~/observatory && git pull");

    // tree_walker
    let tw_current = tree_walker_installed_version();
    let tw_latest = latest_version(&client, TREE_WALKER_REPO).await.ok();
    print_status("tree_walker", tw_current.as_deref(), tw_latest.as_deref(), "epm runtime upgrade tree_walker");
...
fn print_status(name: &str, current: Option<&str>, latest: Option<&str>, upgrade_hint: &str) {
    ...
    let up_to_date = match (current, latest) {
        (Some(c), Some(l)) => Version::parse(c).ok() >= Version::parse(l).ok(),
        _ => false,
    };
    ...
    if current.is_none() {
        ...
        println!("\x1b[32m✓\x1b[0m \x1b[1m{name}\x1b[0m \x1b[2mv{}\x1b[0m", current.unwrap());
    } else {
        println!(
            "\x1b[33m!\x1b[0m \x1b[1m{name}\x1b[0m \x1b[2mv{}  →  v{latest_str} available\x1b[0m",
            current.unwrap()
        );
